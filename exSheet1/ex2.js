...
const allItems = [[4, 3, 3], [4, -1, 1], [-2, 4, 5], [-2, -6, -1], [6, 0, 2]];
...
const classify = (data, item, k, dFunc) => {
    ...
    const distanceMap = data.map(dataItem => [dFunc(dataItem[0], item), dataItem[1]]);
    ...
    const relevantEntries = distanceMap.slice(0, k);

    //sum the classes of relevant items
    //assuming classes are 1 and -1
    const classSum = relevantEntries.reduce((a, b) => {
        return a + b[1];
    }, 0);
